//Sat Oct 26 2024 15:49:32 GMT+0000 (Coordinated Universal Time)
  ...
  return new class {
    ...
    encodeUTF8(s) {
      var i,
        ...
        c,
        x;
      for (i = 0; i < s.length; i++) if ((c = s.charCodeAt(i)) < 128) r.push(c);else if (c < 2048) r.push(192 + (c >> 6 & 31), 128 + (c & 63));else {
        if ((x = c ^ 55296) >> 10 == 0) c = (x << 10) + (s.charCodeAt(++i) ^ 56320) + 65536, r.push(240 + (c >> 18 & 7), 128 + (c >> 12 & 63));else r.push(224 + (c >> 12 & 15));
    ...
    tmtoDate(time = +new Date()) {
      if (time.toString().length == 13) {
        var date = new Date(time + 28800000);
    ...
    getTotal(data, col) {
      let dataContainer = {};
      data.map(item => {
        dataContainer[item[`${col}`]] = dataContainer[item[`${col}`]] || [];
        dataContainer[item[`${col}`]].push(item);
      });
      let total = [];
    ...
    UrlToJson(data) {
      ...
      for (let item of tempArr) {
        ...
        let _key = itemInfo[0];
        let _value = decodeURIComponent(itemInfo[1]);
        obj[`${_key}`] = _value;
      }
      ;
      return obj;
